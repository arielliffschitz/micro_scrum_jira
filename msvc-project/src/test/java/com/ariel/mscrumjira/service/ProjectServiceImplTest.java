...
class ProjectServiceImplTest {
	...
	@DisplayName("updateState with ARCHIVED and no active sprints")
	@Test
	void testUpdateStateArchivedNoActiveSprints() {
		Integer projectKey = 1;
		ProjectState state = ProjectState.ARCHIVED;

		when(repository.findByProjectKey(projectKey)).thenReturn(Optional.of(project));
		project.setState(state);
		PersistenceMetadataUtil.BaseEntityUpdateFields(project, token);

		when(projectSprintService.existSprintByProjectKey(projectKey)).thenReturn(false);

		ProjectDto result = service.updateState(projectKey, state, token);
	...
	@Test
	void testUpdateStateArchivedWithActiveSprints() {
		Integer projectKey = 1;
		ProjectState state = ProjectState.ARCHIVED;
		...
		ResponseStatusException  exception = assertThrows(ResponseStatusException.class,
				() -> service.updateState(projectKey, state, token)); 
